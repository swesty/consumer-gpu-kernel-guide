 ...
 *   - TDP: 140W (SoC, including 20-core Grace CPU)
 *
 * GB10-specific tuning rationale:
 *   - Bandwidth-limited at 273 GB/s (6.5x lower than RTX 6000 Pro's 1.79 TB/s)
 *     → Vectorized loads are critical to saturate available bandwidth
 *     → Minimize memory traffic: fuse read-reduce-write in a single pass per phase
 *   - Only 48 SMs → use moderate thread counts to maintain occupancy
 *     → 512 threads/block for hidden_size=4096 (vs 1024 on larger GPUs)
 *     → Allows 4 blocks/SM resident for better latency hiding
 *   - 24 MB L2 cache → weight tensor (8 KB for BF16 dim=4096) stays fully cached
 *   - LPDDR5X has higher latency than GDDR7/HBM3
 *     → Extra unroll depth to overlap compute with memory stalls
 *
...
// ---------------------------------------------------------------------------
// Constants tuned for GB10 (48 SMs, 273 GB/s LPDDR5X)
// ---------------------------------------------------------------------------
constexpr int WARP_SIZE   = 32;

// Use 512 threads for hidden_size=4096 on GB10:
//   - 4096 / 2 = 2048 vec elements, 512 threads → 4 elements/thread
//   - At 512 threads/block, up to 4 blocks can co-reside per SM (2048 threads/SM)
//   - Better latency hiding than 1024 threads with only 2 blocks/SM
constexpr int MAX_THREADS_GB10 = 512;

// Qwen3-8B dimensions
constexpr int QWEN3_HIDDEN = 4096;
constexpr int QWEN3_HEAD   = 128;
...
//
// GB10 tuning: extra unroll depth (8) to hide LPDDR5X latency.
// With 512 threads and hidden_size=4096: 2048 vec elements / 512 = 4 per thread,
// so the loop body runs 4 times — fully unrolled.
// ===========================================================================
...
// ===========================================================================
// Launch helpers — thread count tuned for GB10's 48 SMs
// ===========================================================================

static inline int compute_threads_vec2(int hidden_size) {
    // For bf16/fp16: 2 elements per vector load
    // Cap at 512 for GB10 to allow more blocks/SM
    int threads = min(hidden_size / 2, MAX_THREADS_GB10);
...
static inline int compute_threads_vec4(int hidden_size) {
    ...
    int threads = min(hidden_size / 4, MAX_THREADS_GB10);
    threads = max(threads, WARP_SIZE);
    threads = ((threads + WARP_SIZE - 1) / WARP_SIZE) * WARP_SIZE;
    return threads;
}
...
static inline int compute_threads_scalar(int hidden_size) {
    int threads = min(hidden_size, MAX_THREADS_GB10);
    threads = max(threads, WARP_SIZE);
    threads = ((threads + WARP_SIZE - 1) / WARP_SIZE) * WARP_SIZE;
    return threads;
}
