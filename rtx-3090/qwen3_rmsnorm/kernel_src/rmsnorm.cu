/*
 * Vectorized RMSNorm CUDA Kernel — RTX 3090 Ampere (sm_86)
 *
 ...
 *
 * RTX 3090 Ampere (GA102) target parameters:
 *   - Compute capability: sm_86
 *   - SMs: 82
 *   - Memory bandwidth: 936 GB/s (GDDR6X, 384-bit)
 *   - L2 cache: 6 MB
 *   - VRAM: 24 GB GDDR6X
 *   - Shared memory/SM: 100 KB (128 KB combined L1+shared)
 *   - Max threads/SM: 1,536 (48 warps)
 *
 * RTX 3090-specific tuning rationale:
 *   - 512 threads/block for hidden_size=4096:
 *     - 3 blocks/SM × 512 threads = 1,536 threads/SM (100% occupancy)
 *     - 1024 threads would give only 1 block/SM = 66.7% occupancy
 *   - #pragma unroll 4: GDDR6X latency (~100-150 ns) is moderate,
 *     and 3 blocks/SM provides ample latency hiding
 *   - 6 MB L2 cache: weights (~8 KB) stay cached, activations do NOT
 *   - No FP8/FP4 tensor core support (3rd gen, INT8/INT4 only)
 *
...
// ---------------------------------------------------------------------------
// Constants tuned for RTX 3090 Ampere (82 SMs, 1536 threads/SM max)
// ---------------------------------------------------------------------------
constexpr int WARP_SIZE   = 32;

// Use 512 threads for RTX 3090 (sm_86, max 1536 threads/SM):
//   - 4096 / 2 = 2048 vec elements, 512 threads → 4 elements/thread
//   - At 512 threads/block, 3 blocks co-reside per SM (1536 threads/SM = 100%)
//   - 1024 threads would give only 1 block/SM = 66.7% occupancy
...
// ===========================================================================
// Launch helpers — thread count tuned for RTX 3090's sm_86 (1536 threads/SM)
// ===========================================================================

static inline int compute_threads_vec2(int hidden_size) {
    // For bf16/fp16: 2 elements per vector load
    // Cap at 512 for 100% occupancy on sm_86
