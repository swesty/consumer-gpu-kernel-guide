# RTX 5090 Blackwell Optimization Guide for CUDA Kernels

Deep dive into RTX 5090 Blackwell (GB202) specific optimizations for LLM inference CUDA kernels, with focus on Qwen3-8B.

## RTX 5090 Blackwell Architecture Overview
...
| Tensor Cores | 680 | 5th gen, FP4/FP8/FP16/BF16 |
| RT Cores | 170 | 4th gen |
| L2 Cache | 96 MB | Larger than most GPUs' VRAM |
| Shared Memory | 128 KB/SM | Configurable up to 228 KB |
| Registers | 64K 32-bit/SM | 255 per thread max |
| Memory | 32 GB GDDR7 | 512-bit interface |
...
| TDP | 575W | Active cooling required |
| PCIe | Gen 5 x16 | ~64 GB/s bidirectional |
| Base/Boost Clock | 2017 / 2407 MHz | Higher base, lower boost than RTX 6000 Pro |

### Blackwell Family Comparison

| Spec | RTX 5090 | RTX 6000 Pro | RTX 5080 | GB10 (DGX Spark) | B200 (Data Center) |
|------|----------|-------------|----------|-------------------|---------------------|
| SMs | **170** | 188 | 84 | 48 | 160 |
...
4. **Large L2** — 96 MB enables caching of most working sets
...
| Model | BF16 Size | Fits in 32 GB? | Remaining for KV/Activations | Notes |
|-------|-----------|----------------|------------------------------|-------|
| Qwen3-8B BF16 | ~16 GB | Yes | ~16 GB | Comfortable fit |
| Qwen3-8B FP8 | ~8 GB | Yes | ~24 GB | Extra room for long context |
| Qwen3-8B INT4 (GPTQ/AWQ) | ~4 GB | Yes | ~28 GB | Maximum headroom |
| Qwen3-14B BF16 | ~28 GB | Tight | ~4 GB | Possible but KV cache limited |
| Qwen3-14B FP8 | ~14 GB | Yes | ~18 GB | Recommended approach |
| Qwen3-30B BF16 | ~60 GB | **No** | — | Requires quantization |
| Qwen3-30B FP8 | ~30 GB | Tight | ~2 GB | Barely fits, minimal KV |
| Qwen3-30B INT4 | ~15 GB | Yes | ~17 GB | Best option for 30B |
| Qwen3-72B | ~144 GB | **No** | — | Cannot fit even quantized |

**Key takeaways:**
- Qwen3-8B BF16 fits comfortably — **this is the sweet spot** for RTX 5090
- Qwen3-14B needs FP8 or INT4 quantization for practical use
- Qwen3-30B requires aggressive quantization (INT4) or cannot run
- The RTX 6000 Pro (96 GB) has no such constraints for models up to ~30B in BF16

### KV Cache Budget at 32 GB

For Qwen3-8B BF16 (~16 GB model weights), ~16 GB remains for KV cache:
...
Available: ~16 GB = ~16,384 MB
Max tokens: 16,384 MB / 0.144 MB ≈ 113,778 tokens

With activations overhead (~2-4 GB):
  Available for KV: ~12-14 GB
  Max tokens: ~83,000 - 97,000 tokens
```

This is sufficient for most inference workloads. For extremely long contexts (>100K tokens), consider FP8 KV cache quantization (halves KV memory) or use the FP8 model variant.

## Memory Hierarchy Optimization

### The RTX 5090 Memory Stack
...
L2 Cache:       96 MB unified         │  ~200 cycles  │  Global shared
GDDR7:          32 GB                 │  ~400 cycles  │  Global
...
| L2 Cache | ~8-12 TB/s | ~200 cycles | 96 MB |
| GDDR7 | **1.79 TB/s** | ~400 cycles | 32 GB |
...
### L2 Cache Exploitation (96 MB)

The 96 MB L2 is smaller than the RTX 6000 Pro's 128 MB but still a major advantage over most GPUs:

| Working Set | Size | Fits in 96 MB L2? |
|-------------|------|---------------------|
...
| RMSNorm input [4, 4096, 4096] BF16 | 128 MB | No (just over) |
| KV cache (seq=512, all layers) | ~600 MB | No |
| Attention scores (one layer, seq=2048) | ~32 MB | Yes |

Compared to RTX 6000 Pro: the 96 MB L2 means some working sets that fit in 128 MB will spill. For example, a [4, 4096, 4096] BF16 activation tensor (128 MB) fits in the RTX 6000 Pro's L2 but not the RTX 5090's. In practice, the difference is minor for most inference workloads.
...
### Block Size Selection for RTX 5090

With 170 SMs and high bandwidth, maximize per-block throughput:
...
**Why 1024 works on RTX 5090 (but not GB10):**
...
### Grid Sizing for 170 SMs

For maximum GPU utilization, `num_blocks` should be >= 170, ideally a multiple:

```
RMSNorm grid = num_rows (one block per row):
  seq_len=1:     1 row    → 1/170 SMs (0.6% util) — poor
  seq_len=128:   128 rows → 75% SM utilization
  seq_len=170:   170 rows → 100% SM utilization (ideal)
  seq_len=512:   512 rows → 3.0 blocks/SM (saturated)
  seq_len=2048:  2048 rows → 12.0 blocks/SM (fully saturated)
```

Short sequences (< 170 rows) underutilize the GPU. For single-token decode, the RTX 5090's 170 SMs are mostly idle on a per-kernel basis — overall throughput comes from pipelining across the model's many layers.
...
```
Peak FP32: ~90 TFLOPS (estimated, fewer SMs than RTX 6000 Pro)
Peak BW:   1.79 TB/s
Crossover: 90000 / 1790 ≈ 50 FLOP/byte

Operations below 50 FLOP/byte are memory-bound.
```
...
For a well-optimized RMSNorm kernel on RTX 5090:
...
- **Critical for RTX 5090**: FP8/FP4 quantization extends model fit within 32 GB VRAM
...
## Qwen3-8B on RTX 5090: Optimization Priorities

### Model Fit

Qwen3-8B in BF16 (~16 GB) leaves ~16 GB of GDDR7 free for:
- KV caches (long context inference)
- Batch processing (moderate-throughput serving)
- Activations during inference

Unlike the RTX 6000 Pro (80 GB free), the RTX 5090 has tighter headroom. For high-throughput serving with large KV caches, consider FP8 model weights to free additional VRAM.
...
On RTX 5090, RMSNorm is a smaller fraction of total time than on GB10 (because linear projections run 6.5x faster with 6.5x more bandwidth, shifting the bottleneck toward compute). Custom kernels have relatively less end-to-end impact, but the kernel itself still runs proportionally faster.
...
The RTX 5090 is an excellent target for `torch.compile`:
```python
...
## Profiling on RTX 5090

### NVIDIA Nsight Systems (nsys)

```bash
nsys profile -o rtx5090_profile python your_script.py
...
5. **L2 eviction on large batches**
   - Batch x seq x hidden > 96 MB -> input data doesn't fit in L2
   - Streaming access for non-reusable data (`__ldcs`)
   - Note: threshold is lower than RTX 6000 Pro (96 MB vs 128 MB)

6. **VRAM exhaustion**
   - 32 GB fills up faster than RTX 6000 Pro's 96 GB
   - Monitor with `torch.cuda.memory_allocated()` and `torch.cuda.memory_reserved()`
   - Use FP8/INT4 quantization for larger models
   - Consider gradient checkpointing if fine-tuning

## Best Practices Summary for RTX 5090
...
4. **Exploit 96 MB L2** — weight tensors and moderate-sized activations stay cached.
...
8. **BF16 is the default for 8B models** — 32 GB GDDR7 fits Qwen3-8B comfortably; use FP8/INT4 for larger models.
9. **Use explicit type conversions** — `to_float()` / `from_float()` helpers for PyTorch compat.
10. **Grid size >= 170** — keep all SMs busy; short sequences underutilize the GPU.
...
On RTX 5090, torch.compile's benefit comes primarily from fusing Python dispatch overhead and eliminating kernel launch gaps — the 1.79 TB/s bandwidth means individual kernels are already fast, so reducing the gaps between them matters.

## CUDA Graphs

CUDA graphs capture a sequence of kernel launches and replay them with near-zero CPU overhead. On RTX 5090, individual kernels complete very quickly (high bandwidth), making the relative cost of per-kernel CPU dispatch proportionally higher.
...
### RTX 5090-Specific Benefit

With 1.79 TB/s bandwidth, a small RMSNorm kernel ([1, 1, 4096] BF16) executes in ~10-20 us. CPU launch overhead of ~5-10 us per kernel is a significant fraction. CUDA graphs eliminate this overhead entirely, which matters most for:
...
huggingface-cli upload your-username/qwen3-rmsnorm-rtx5090 ./dist
```
...
## RTX 5090 as a Consumer GPU: Practical Considerations

### No ECC Memory

Unlike the RTX 6000 Pro (which has ECC GDDR7), the RTX 5090 does not have ECC. For inference workloads, this is rarely an issue — bit flips are extremely rare and inference is not safety-critical in most deployments. For scientific computing or long-running training jobs, consider the RTX 6000 Pro.

### No NVLink — Single-GPU Only

The RTX 5090 does not support NVLink. Multi-GPU setups use PCIe, which limits inter-GPU bandwidth to ~64 GB/s (vs NVLink's 900 GB/s on data center GPUs). For tensor parallelism across multiple RTX 5090s, PCIe becomes the bottleneck. In practice, most RTX 5090 deployments are single-GPU.

### Power and Cooling

At 575W TDP, the RTX 5090 requires:
- A high-quality PSU (850W+ recommended for system)
- Good case airflow or liquid cooling
...
```
With only 32 GB VRAM, this is more common than on RTX 6000 Pro. Solutions:
- Use FP8 or INT4 quantized models
- Reduce batch size
...
|---------|-------------|----------------|
| **TCGen05 (5th-gen Tensor Cores)** | 256x256x16 MMA spanning 2 SMs via CTA pairs | Most kernels use Hopper-era 128x128 tiles |
| **Tensor Memory (TMEM)** | 128x512 x 32-bit on-chip buffer dedicated to tensor core data | Kernels not using TMEM leave bandwidth on the table |
...
Kernels tuned for SM90 (Hopper) that use `wgmma` with 128x128 tiles, standard shared memory, and per-warp TMA are not exploiting Blackwell's architectural advances. The RTX 5090 with 170 SMs provides 85 CTA pairs for cooperative execution.
...
On RTX 5090, look for: GPU idle gaps between kernels (fusion opportunities), low DRAM throughput below 35% of 1.79 TB/s (coalescing issues), high kernel launch overhead relative to fast kernel execution (CUDA graph candidates).
...
On RTX 5090 at 1.79 TB/s, individual kernels complete very quickly — but the launch overhead between them becomes proportionally significant. Fusing 3 kernels into 1 eliminates ~10-20 us of launch overhead plus two intermediate memory round-trips.

#### 3. Exploit Blackwell-Specific Hardware

For GEMM-class kernels on Blackwell:
- Use **CTA-pair 256x256 tiles** instead of Hopper's 128x128 — doubles the work per tensor core instruction. The RTX 5090's 170 SMs provide 85 CTA pairs.
- Use **TMEM** for tensor core input staging instead of shared memory — dedicated on-chip buffer with higher throughput
- Use **TMA multicast** to load data into multiple SMs' shared memory simultaneously — reduces redundant global memory reads. Particularly effective with 96 MB L2 cache serving as the multicast source.
...
- The RTX 5090's 96 MB L2 makes inter-kernel data reuse likely — fused kernels that keep intermediate results in registers are still better, but the penalty for unfused sequential kernels is lower than on GPUs with smaller L2
...
Note: On RTX 5090, tensor parallelism across multiple GPUs is limited by PCIe bandwidth (~64 GB/s). Single-GPU inference is strongly preferred.
...
Fusing quantization into compute kernels (e.g., `rms_norm + fp8_quant`) eliminates a separate quantization kernel launch and its associated memory traffic. This is especially valuable on RTX 5090 where FP8/FP4 quantization may be needed to fit larger models in 32 GB.
...
│ Peak BW:  1792 GB/s (realistic: ~650)   │
├─────────────────────────────────────────┤
│ VRAM:     32 GB — plan model fit!       │
│ Qwen3-8B BF16: ~16 GB (fits)           │
│ Qwen3-14B: use FP8 (~14 GB)            │
│ Qwen3-30B: use INT4 (~15 GB)           │
└─────────────────────────────────────────┘
```
